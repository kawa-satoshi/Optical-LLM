...
analog_convert(
    ...
    out_bit=8,
    std=0.0,
    targets=["lora_A", "lora_B", "base_layer"],
    convert_linear=True,
    convert_conv2d=False,
    type="affine",
    verbose=True,
)
print(lora_model)
            ...
            (0-11): 12 x ViTLayer(
              (attention): ViTAttention(
                (attention): ViTSelfAttention(
                  (query): lora.Linear(
                    (base_layer): AffineQuantizeLinear(
                      in_features=768, out_features=768, bias=True
                      (normalizer): DoNothing()
                      (quantizer_in): AffineQuantize(bitwidth=8)
                      (quantizer_w): AffineQuantize(bitwidth=8)
                      (quantizer_out): AffineQuantize(bitwidth=8)
                      (noiser): GaussianNoise(std=0.0)
                    )
                    (lora_dropout): ModuleDict(
                      (default): Dropout(p=0.1, inplace=False)
                    )
                    (lora_A): ModuleDict(
                      (default): AffineQuantizeLinear(
                        in_features=768, out_features=8, bias=False
                        (normalizer): DoNothing()
                        (quantizer_in): AffineQuantize(bitwidth=8)
                        (quantizer_w): AffineQuantize(bitwidth=8)
                        (quantizer_out): AffineQuantize(bitwidth=8)
                    ...
                    (lora_B): ModuleDict(
                      (default): AffineQuantizeLinear(
                        in_features=8, out_features=768, bias=False
                        (normalizer): DoNothing()
                        (quantizer_in): AffineQuantize(bitwidth=8)
                        (quantizer_w): AffineQuantize(bitwidth=8)
                        (quantizer_out): AffineQuantize(bitwidth=8)
...
{'eval_loss': 0.24971896409988403, 'eval_accuracy': 0.952, 'eval_runtime': 5.5947, 'eval_samples_per_second': 89.37, 'eval_steps_per_second': 0.357, 'epoch': 4.44}
{'train_runtime': 231.129, 'train_samples_per_second': 97.348, 'train_steps_per_second': 0.087, 'train_loss': 1.3003050684928894, 'epoch': 4.44}
"""
